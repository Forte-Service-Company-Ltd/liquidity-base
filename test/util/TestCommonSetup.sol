...
abstract contract TestCommonSetup is TestCommonSetupAbs {
    ...
    function _setupPool(bool withStableCoin) internal endWithStopPrank returns (PoolBase poolRet) {
        ...
        poolRet = _deployPool(address(xToken), yTokenAddress, 30, X_TOKEN_MAX_SUPPLY, INACTIVE_LIQUIDITY, TBCInputOption.BASE);
    ...
    function _setupDuplicatePools(
        bool withStableCoin,
        uint16 lpFee,
        uint wInactive
    ) internal endWithStopPrank returns (PoolBase pool1, PoolBase pool2) {
        _setUpTokensAndFactories(X_TOKEN_MAX_SUPPLY);
        _approveFactory(address(xToken));
        address yTokenAddress = withStableCoin ? address(stableCoin) : address(yToken);
        pool1 = _deployPool(address(xToken), yTokenAddress, lpFee, X_TOKEN_MAX_SUPPLY, wInactive, TBCInputOption.BASE);
        _approvePool(pool1, false);
        pool2 = _deployPool(address(xToken), yTokenAddress, lpFee, X_TOKEN_MAX_SUPPLY, wInactive, TBCInputOption.BASE);
    ...
    function _setupPoolWithFee(
        ...
        address yTokenAddress = withStableCoin ? address(stableCoin) : address(yToken);
        poolRet = _deployPool(_xTokenAddress, yTokenAddress, fee, X_TOKEN_MAX_SUPPLY, INACTIVE_LIQUIDITY, TBCInputOption.BASE);
        _approvePool(poolRet, false);
    }
    ...
    function _setupPoolForkTest(
        ...
        poolRet = PoolBase(
            _deployPool(address(xTokenWithFee), _yTokenAddress, 0, 10e3 * ERC20_DECIMALS, INACTIVE_LIQUIDITY, TBCInputOption.FORK)
        );
    ...
    function _setupStressTestPool(bool withStableCoin) internal endWithStopPrank returns (PoolBase poolRet) {
        ...
        poolRet = _deployPool(address(xToken), yTokenAddress, 0, maxX, INACTIVE_LIQUIDITY, TBCInputOption.FORK);
        _approvePool(poolRet, false);
        // _addInitialLiquidity(poolRet, 10e3 * ERC20_DECIMALS);
    }
    ...
    function _setupPrecisionPools(
        ...
        wadPool = _deployPool(address(xToken), address(yToken), fee, maxSupply, INACTIVE_LIQUIDITY, TBCInputOption.PRECISION);
        ...
        sixDecimalPool = _deployPool(address(xToken), address(stableCoin), fee, maxSupply, INACTIVE_LIQUIDITY, TBCInputOption.PRECISION);
        _approveFactory(address(xToken));
        _loadAdminAndAlice();
    ...
    function _setupFOTPool(bool withStableCoin) internal endWithStopPrank returns (PoolBase poolRet) {
        ...
        address yTokenAddress = withStableCoin ? address(stableCoin) : address(yToken);
        poolRet = _deployPool(address(fotCoin), yTokenAddress, 30, X_TOKEN_MAX_SUPPLY, INACTIVE_LIQUIDITY, TBCInputOption.BASE);
        _approvePool(poolRet, false);
        pool = poolRet;
        _setupCollateralToken();
    }
